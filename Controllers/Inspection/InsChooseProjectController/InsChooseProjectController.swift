...
class InsChooseProjectController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource{
	// MARK: Variables
	...
	@IBAction func clickConfirm(_ sender: Any) {
		if characterIndex == -1 || projectIndex == -1 {
			...
			self.navigationController?.pushViewController(InsDashboardController(), animated: true)
