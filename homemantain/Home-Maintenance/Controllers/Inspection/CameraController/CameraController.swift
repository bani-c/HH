...
class CameraController: UIViewController
{
    ...
    fileprivate func setupToolBar() {
		...
		toolBar.loadButton?.isHidden = false
		toolBar.loadButton?.setTitle("相簿", for: UIControlState())
    ...
    fileprivate func setupCanvas() {
		...
		let top = max(self.view.safeAreaInsets.top, 70.0) + margin
		let bottom = (self.toolBar?.frame.minY ?? self.view.bounds.maxY) - margin
		canvasView.frame = CGRect(
			x: margin,
			y: top,
			width: self.view.bounds.width - (margin * 2.0),
    ...
    fileprivate func presentImagePickerController() {
        if UIImagePickerController.isSourceTypeAvailable(.camera) {
			...
			// Simulator and devices without a camera can still test the complete
			// photo flow by selecting an image from the photo library.
			self.showPhotoLibrary()
        }
    }
