...
class InsTmpDataManager {
    ...
    func clearUploadDB() {
        do {
            ...
            let building = UserDefaults.standard.string(forKey: "BUILDING")!
            //let fileName = String.init(format: SystemConstants.DBFileNameSub, projectsNo, building)
            let fileName = "HHSub_102_C_Upload.db"
            let fileURL = documentDirectory.appendingPathComponent(fileName)
                ...
                DispatchQueue.main.sync {
                    do {
                        ...
                        if status == "3000" {
                            ...
                            self.upPicIndex += 1
                            self.hud?.indicatorView = JGProgressHUDPieIndicatorView()
                            let progress = self.upPicArr.isEmpty ? 1.0 : Float(self.upPicIndex) / Float(self.upPicArr.count)
                            self.hud?.progress = min(progress, 1.0)
                            self.hud?.detailTextLabel.text = String.init(format: "%d%%", Int(min(progress, 1.0) * 100))
                            self.hud?.textLabel.text = "圖片上傳中..."
                            if self.upPicIndex < self.upPicArr.count {
                                self.uploadPic(self.upPicArr[self.upPicIndex], dbId: self.fileIdStr, view: view)
                            } else {
                                self.finishCurrentUpload(view: view)
                            }
    ...
    private func finishCurrentUpload(view: UIView) {
        self.hud?.progress = 1.0
        self.hud?.detailTextLabel.text = "100%"
        self.hud?.textLabel.text = "圖片上傳完成"

        DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(300)) {
            self.hud?.dismiss()

            let needUpdateFiles = UserDefaults.standard.object(forKey: "NeedUpdateArr") as? [String] ?? []
            let nextIndex = UserDefaults.standard.integer(forKey: "NeedUpdateArrIndex") + 1

            if nextIndex < needUpdateFiles.count {
                UserDefaults.standard.set(nextIndex, forKey: "NeedUpdateArrIndex")
                self.uploadSqlDB(needUpdateFiles[nextIndex], view: view)
            } else {
                UserDefaults.standard.set(0, forKey: "NeedUpdateArrIndex")
                let alert = UIAlertController(title: "", message: "上傳完成", preferredStyle: .alert)
