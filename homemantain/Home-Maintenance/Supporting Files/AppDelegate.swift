...
class AppDelegate: UIResponder, UIApplicationDelegate {
	...
	func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
		...
		do {
			...
			let name = Expression<String?>("userid")
			let pwd = Expression<String?>("userpwd")
			
			for user in try db.prepare(users) {
				print("name: \(String(describing: user[name])), pwd: \(String(describing: user[pwd]))")
			...
			let query = projects.select(project_name, project_no)
				.order(project_sorting.asc) // ORDER BY "email" DESC, "name"
        ...
        //InsTmpDataManager.sharedInstance().uploadSqlDB("HHSub_B01_L_Upload_t.sql")
	...
	lazy var persistentContainer: NSPersistentContainer = {
	    ...
	    */
	    let container = NSPersistentContainer(name: "Home_Maintenance")
