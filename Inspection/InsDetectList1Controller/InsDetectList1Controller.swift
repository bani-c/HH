   ...
   // var dataArea0 = [] as [InsAreaItem]
	...
	func getLocalData() -> Bool {
		if InsTmpDataManager.sharedInstance().dicArea["Ins_DataArea"] != nil {
           ...
           // dataArea0 = InsTmpDataManager.sharedInstance().dicArea["Ins_DataArea0"] ?? []
...
//                        }
    ...
    func initDataReinspection0() {
        do {
            ...
            for insAreaItem in dataArea {
                ...
                for insPlaceItem in insAreaItem.places {
                    ...
                    let queryItem = InspDetail.select(InspDescItemId, InspRemark, SeqNo, ProjInspIdx, Result, CheckEquipType, EquipFailLessAmount, EquipFailType).filter(ELEVEL_2_1 == building && ELEVEL_2_2 == room && ELEVEL_1 == floor && Result == "N" && IsValid == "Y" && ChkNo == targetChkNo && AreaId == insAreaItem.idx && InspPlaceId == insPlaceItem.idx).order(Sorting.asc)
                    for data in try db.prepare(queryItem) {
       ...
       // InsTmpDataManager.sharedInstance().dicArea["Ins_DataArea0"] = dataArea0
